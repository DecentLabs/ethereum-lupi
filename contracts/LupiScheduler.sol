...
contract LupiScheduler is owned, usingOraclize {
    ...
    function LupiScheduler(uint _oraclizeGasPrice, uint _gasStartRevealing, uint _gasRevealPerTicket,
    ...
    function __callback(bytes32 queryId, string resultRcv) {
        ...
        if (res == START_REVEALING_HASH ) {
            ...
            scheduleDeclareWinner(m_queries[queryId]);

        } else if (res == DECLARE_WINNER_HASH ) {
            gameInstance.declareWinner();
            // TODO:
